...
// Defaults are friendly to XAMPP/WAMP local MySQL installations.
const config = {
...
async function initDatabase() {
  ...
  // First connect without selecting a database so the server can create it.
  ...
  // Create/upgrade tables automatically. This removes the need to manually
  // import SQL before starting the server.
...
async function seedDemoData() {
  ...
  for (const p of plans) {
    await execute(
      `INSERT INTO membership_plans(name,duration_months,price,description)
       VALUES(?,?,?,?) ON DUPLICATE KEY UPDATE duration_months=VALUES(duration_months),price=VALUES(price),description=VALUES(description)`, p
    );
  }
  ...
  } else membershipId = membershipRows[0].id;
  ...
  for (const [title, description, start, end, capacity, room, days] of classData) {
    const [existing] = await execute('SELECT id FROM fitness_classes WHERE title=? AND class_date=DATE_ADD(CURDATE(), INTERVAL ? DAY) LIMIT 1', [title, days]);
    if (!existing.length) {
      await execute(
  ...
  if (!booking.length) await execute('INSERT INTO bookings(class_id,user_id,status) VALUES(?,?,?)', [firstClass.id, member.id, 'booked']);

  const [payment] = await execute('SELECT id FROM payments WHERE reference=? LIMIT 1', ['FH-DEMO-001']);
  if (!payment.length) await execute(
  ...
  if (!progress.length) {
    await execute(`INSERT INTO workout_progress(user_id,metric_date,weight_kg,body_fat,chest_cm,waist_cm,notes)
      VALUES(?,DATE_SUB(CURDATE(),INTERVAL 30 DAY),71.50,18.20,96,82,'Starting measurement'),
            (?,CURDATE(),70.80,17.40,98,80,'Improved strength and consistency')`, [member.id, member.id]);
  }
}
